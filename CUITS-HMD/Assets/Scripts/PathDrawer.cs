...
public class PathDrawer : MonoBehaviour
{
  ...
  public float tileSize = 0.0344f; // adjust based on your grid
  public Vector3 offset = new Vector3(-0.49f, 0.105f, -1f); // position offset for the entire path
  public float aspectRatio = 0.456f;
...
void Start()
{
       if (lineRenderer != null)
       {
           ...
           lineRenderer.material.color = lineColor; // <-- This line is important!
           ToggleVisibility();
   }
}
   ...
   public void DrawPath(List<Vector2Int> path)
   {
       ...
       for (int i = 0; i < path.Count; i++)
       {
           ...
           Vector3 position = new Vector3(x * tileSize, -y * tileSize * aspectRatio, .1f); // lifted slightly for visibility
   ...
   // Add this method to your PathDrawer class
   public void ToggleVisibility()
   {
       if (lineRenderer != null)
       {
           // Toggle the enabled state of the LineRenderer
           lineRenderer.enabled = !lineRenderer.enabled;
           Debug.Log($"PathDrawer: Line visibility toggled to {(lineRenderer.enabled ? "visible" : "hidden")}");
       }
       else
       {
           Debug.LogError("PathDrawer: Cannot toggle visibility - LineRenderer is null");
       }
   }
}
