...
public class CompassBar2 : MonoBehaviour
{
    ...
    private float viewBasedHeading = 0f;
    private float tssHeading = 0f;
    private float headingLerpSpeed = 10f;


    private float alignmentTimer = 0f;
    private float alignmentInterval = 5f; // Align with TSS every 5 seconds
    private bool isAligning = false;
    private float alignmentDuration = 1f; // Time taken to align with TSS
    private float alignmentProgress = 0f;
    private float headingOffset = 0f; // Offset between view direction and TSS heading
    ...
    private async void LateUpdate()
    {
        ...
        viewBasedHeading = forwardSignedAngle;
        if (viewBasedHeading < 0)
        {
            viewBasedHeading += 360f;
        }

        // Check if it's time to align with TSS
        if (alignmentTimer >= alignmentInterval)
        {
            alignmentTimer = 0f;
            
            // Update the offset without triggering a re-scroll
            float currentHeading = (viewBasedHeading + headingOffset) % 360f;
            float newOffset = headingOffset + Mathf.DeltaAngle(currentHeading, tssHeading);
            headingOffset = newOffset;
        }

        // Always use view direction plus offset
        heading = (viewBasedHeading + headingOffset) % 360f;
