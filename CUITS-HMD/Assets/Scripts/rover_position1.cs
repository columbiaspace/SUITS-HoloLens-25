...
public class rover_position1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log($"[rover_position] Start - Initial position: {initialPosition}");

        // Check if BackendDataService exists
        if (BackendDataService.Instance == null)
        {
            Debug.LogError("[rover_position] ERROR: No BackendDataService found in scene! Please add BackendDataService component to a GameObject.");
        }
    }
    ...
    void Update()
    {
        ...
        {
            float posx = BackendDataService.Instance.LatestData.rover.posx ;
            float posy = BackendDataService.Instance.LatestData.rover.posy ;
            ...
            if (posx != 0 || posy != 0) 
            {
                ...
                {
                    Debug.Log($"[rover_position] First position received: posx={posx}, posy={posy}");
        ...
        {
            Debug.LogWarning("[rover_position] Waiting for valid position from BackendDataService...");
        }
    }
}